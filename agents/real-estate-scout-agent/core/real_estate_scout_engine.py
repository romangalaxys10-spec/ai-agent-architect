"""
Real Estate Scout Engine — Real Estate Scout.
Location scoring, comps analysis, cap-rate math, due-diligence checklist
...
class RealEstateScoutEngine:
    """Deterministic engine for Real Estate Scout — zero network, pure logic."""
    ...
    def analyze(cls, text: str, **kwargs) -> Analysis:
        ...
        # --- Domain-specific heuristics for Real Estate Scout ---

        # Heuristic 1: keyword coverage for real estate scout
        domain_keywords = ['real', 'estate', 'scout']
        hits = sum(1 for kw in domain_keywords if kw in low)
        if hits == 0 and len(low) > 20:
            findings.append(Finding("real-estate-scout-agent","medium","Low Real Estate Scout signal","No core keywords (" + ", ".join(domain_keywords) + ") detected in content","Add explicit coverage of real or confirm scope is out-of-domain"))
        metrics["keyword_hits"] = hits

        # Heuristic 2: length / depth check
        if metrics["words"] < 15:
            findings.append(Finding("real-estate-scout-agent","low","Thin content","Very short input — analysis will be shallow","Provide 3-5 representative paragraphs or a file for deeper analysis"))
        ...
        if found_risks:
            findings.append(Finding("real-estate-scout-agent","medium","Unresolved markers","Found placeholders: "+ ", ".join(found_risks),"Resolve or ticket them before ship"))
        ...
        if any(w in low for w in ["critical","urgent","blocker","p0","sev1"]):
            findings.append(Finding("real-estate-scout-agent","high","Urgency signal present","Input declares critical/urgent scope","Confirm severity, add mitigation owner and deadline"))
            metrics["urgency"] = True

        # Heuristic 5: structure check — does input look structured?
        if ":" not in t and "\n" not in t.strip():
            findings.append(Finding("real-estate-scout-agent","low","Unstructured input","Single-line / no key:value structure detected","Provide structured input (bullets, JSON, or sections) for richer output"))
        ...
        if "Ops, Finance & Legal" in ["Security & Compliance","Ops, Finance & Legal"] and "compliance" not in low and "risk" not in low:
            findings.append(Finding("real-estate-scout-agent","low","Compliance language absent","No compliance/risk framing found for a Ops, Finance & Legal task","State assumptions, scope, and non-advice disclaimer where needed"))
