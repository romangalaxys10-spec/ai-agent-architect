"""CLI for Bug Bounty Triage — Report deduplication, reproducibility checks, severity re-grading, payout band mapping"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Bug Bounty Triage — Report deduplication, reproducibility checks, severity re-grading, payout band mapping')
    ...
    from core.bug_bounty_triage_engine import BugBountyTriageEngine
    analysis = BugBountyTriageEngine.analyze(text)
    if args.json:
        ...
        print(BugBountyTriageEngine.format_report(analysis))
