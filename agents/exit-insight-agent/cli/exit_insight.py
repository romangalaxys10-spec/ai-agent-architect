"""CLI for Exit Insight Miner — Departure theme extraction, regretted vs non-regretted loss, retention lever mapping"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Exit Insight Miner — Departure theme extraction, regretted vs non-regretted loss, retention lever mapping')
    ...
    from core.exit_insight_engine import ExitInsightEngine
    analysis = ExitInsightEngine.analyze(text)
    if args.json:
        ...
        print(ExitInsightEngine.format_report(analysis))
