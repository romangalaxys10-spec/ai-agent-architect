"""
Insurance Advisor Engine — Insurance Coverage Auditor.
Gap analysis, limit adequacy, exclusion flagging, renewal negotiation prep (not advice)
...
class InsuranceAdvisorEngine:
    """Deterministic engine for Insurance Coverage Auditor — zero network, pure logic."""
    ...
    def analyze(cls, text: str, **kwargs) -> Analysis:
        ...
        # --- Domain-specific heuristics for Insurance Coverage Auditor ---

        # Heuristic 1: keyword coverage for insurance advisor
        domain_keywords = ['insurance', 'advisor']
        hits = sum(1 for kw in domain_keywords if kw in low)
        if hits == 0 and len(low) > 20:
            findings.append(Finding("insurance-advisor-agent","medium","Low Insurance Coverage Auditor signal","No core keywords (" + ", ".join(domain_keywords) + ") detected in content","Add explicit coverage of insurance or confirm scope is out-of-domain"))
        metrics["keyword_hits"] = hits

        # Heuristic 2: length / depth check
        if metrics["words"] < 15:
            findings.append(Finding("insurance-advisor-agent","low","Thin content","Very short input — analysis will be shallow","Provide 3-5 representative paragraphs or a file for deeper analysis"))
        ...
        if found_risks:
            findings.append(Finding("insurance-advisor-agent","medium","Unresolved markers","Found placeholders: "+ ", ".join(found_risks),"Resolve or ticket them before ship"))
        ...
        if any(w in low for w in ["critical","urgent","blocker","p0","sev1"]):
            findings.append(Finding("insurance-advisor-agent","high","Urgency signal present","Input declares critical/urgent scope","Confirm severity, add mitigation owner and deadline"))
            metrics["urgency"] = True

        # Heuristic 5: structure check — does input look structured?
        if ":" not in t and "\n" not in t.strip():
            findings.append(Finding("insurance-advisor-agent","low","Unstructured input","Single-line / no key:value structure detected","Provide structured input (bullets, JSON, or sections) for richer output"))
        ...
        if "Ops, Finance & Legal" in ["Security & Compliance","Ops, Finance & Legal"] and "compliance" not in low and "risk" not in low:
            findings.append(Finding("insurance-advisor-agent","low","Compliance language absent","No compliance/risk framing found for a Ops, Finance & Legal task","State assumptions, scope, and non-advice disclaimer where needed"))
