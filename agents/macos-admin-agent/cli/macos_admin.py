"""CLI for macOS Admin — launchd, defaults, profiles, MDM, SIP/TCC, and `systemextensionsctl`"""
import os, sys
...
def main():
    parser=argparse.ArgumentParser(description="macOS Admin — launchd, defaults, profiles, MDM, SIP/TCC, and `systemextensionsctl`")
    ...
    from core.macos_admin_engine import MacosAdminEngine
    analysis=MacosAdminEngine.analyze(text, os_hint=args.os)
    if args.json:
        ...
        print(MacosAdminEngine.format_report(analysis))
