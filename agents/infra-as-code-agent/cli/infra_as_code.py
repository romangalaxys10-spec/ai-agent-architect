"""CLI for IaC & Platform Engineering — Terraform/Pulumi generation, drift detection, env parity checks, plan review"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='IaC & Platform Engineering — Terraform/Pulumi generation, drift detection, env parity checks, plan review')
    ...
    from core.infra_as_code_engine import InfraAsCodeEngine
    analysis = InfraAsCodeEngine.analyze(text)
    if args.json:
        ...
        print(InfraAsCodeEngine.format_report(analysis))
