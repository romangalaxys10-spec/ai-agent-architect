"""CLI for Brand Voice Guardian — Tone taxonomy enforcement, off-brand flagging, rewrite suggestions with voice preservation"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Brand Voice Guardian — Tone taxonomy enforcement, off-brand flagging, rewrite suggestions with voice preservation')
    ...
    from core.brand_voice_engine import BrandVoiceEngine
    analysis = BrandVoiceEngine.analyze(text)
    if args.json:
        ...
        print(BrandVoiceEngine.format_report(analysis))
