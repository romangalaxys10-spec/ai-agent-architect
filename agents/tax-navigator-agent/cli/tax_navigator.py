"""CLI for Tax Navigator — Jurisdiction mapping, nexus flagging, filing deadline calendar, risk-ranked action list (not advice)"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Tax Navigator — Jurisdiction mapping, nexus flagging, filing deadline calendar, risk-ranked action list (not advice)')
    ...
    from core.tax_navigator_engine import TaxNavigatorEngine
    analysis = TaxNavigatorEngine.analyze(text)
    if args.json:
        ...
        print(TaxNavigatorEngine.format_report(analysis))
