"""CLI for Browser Pilot — Tab orchestration, navigation guards, history/cookie/session control, and profile isolation"""
import os, sys
...
def main():
    parser=argparse.ArgumentParser(description="Browser Pilot — Tab orchestration, navigation guards, history/cookie/session control, and profile isolation")
    ...
    from core.browser_pilot_engine import BrowserPilotEngine
    analysis=BrowserPilotEngine.analyze(text, os_hint=args.os)
    if args.json:
        ...
        print(BrowserPilotEngine.format_report(analysis))
