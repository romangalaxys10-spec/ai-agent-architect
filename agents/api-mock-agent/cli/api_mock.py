"""CLI for Mock & Contract Stub Generator — Deterministic mocks from OpenAPI, Pact contracts, stub servers, fake-data factories"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Mock & Contract Stub Generator — Deterministic mocks from OpenAPI, Pact contracts, stub servers, fake-data factories')
    ...
    from core.api_mock_engine import ApiMockEngine
    analysis = ApiMockEngine.analyze(text)
    if args.json:
        ...
        print(ApiMockEngine.format_report(analysis))
