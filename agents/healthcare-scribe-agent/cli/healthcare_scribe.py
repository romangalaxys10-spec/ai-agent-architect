"""CLI for Healthcare Scribe — SOAP note structuring, code suggestion, compliance flagging (not medical advice)"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Healthcare Scribe — SOAP note structuring, code suggestion, compliance flagging (not medical advice)')
    ...
    from core.healthcare_scribe_engine import HealthcareScribeEngine
    analysis = HealthcareScribeEngine.analyze(text)
    if args.json:
        ...
        print(HealthcareScribeEngine.format_report(analysis))
