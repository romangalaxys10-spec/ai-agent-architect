"""CLI for Infra Provisioner — Terraform/OpenTofu, state locking, drift detection, and provider pinning"""
import os, sys
...
def main():
    parser=argparse.ArgumentParser(description="Infra Provisioner — Terraform/OpenTofu, state locking, drift detection, and provider pinning")
    ...
    from core.infra_provisioner_engine import InfraProvisionerEngine
    analysis=InfraProvisionerEngine.analyze(text, os_hint=args.os)
    if args.json:
        ...
        print(InfraProvisionerEngine.format_report(analysis))
