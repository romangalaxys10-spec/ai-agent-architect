"""CLI for Hospitality Ops Concierge — Occupancy pacing, upsell triggers, staffing-to-demand matching, guest recovery workflows"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Hospitality Ops Concierge — Occupancy pacing, upsell triggers, staffing-to-demand matching, guest recovery workflows')
    ...
    from core.hospitality_ops_engine import HospitalityOpsEngine
    analysis = HospitalityOpsEngine.analyze(text)
    if args.json:
        ...
        print(HospitalityOpsEngine.format_report(analysis))
