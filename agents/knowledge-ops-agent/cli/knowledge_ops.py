"""CLI for Knowledge Ops Scaler — Article performance vs ticket deflection, freshness SLA, AI-assist readiness scoring"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Knowledge Ops Scaler — Article performance vs ticket deflection, freshness SLA, AI-assist readiness scoring')
    ...
    from core.knowledge_ops_engine import KnowledgeOpsEngine
    analysis = KnowledgeOpsEngine.analyze(text)
    if args.json:
        ...
        print(KnowledgeOpsEngine.format_report(analysis))
