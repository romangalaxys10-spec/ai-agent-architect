"""CLI for Revenue Forecast Radar — Pipeline coverage math, stage-weighted forecast, risk-adjusted commit calls"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Revenue Forecast Radar — Pipeline coverage math, stage-weighted forecast, risk-adjusted commit calls')
    ...
    from core.forecast_radar_engine import ForecastRadarEngine
    analysis = ForecastRadarEngine.analyze(text)
    if args.json:
        ...
        print(ForecastRadarEngine.format_report(analysis))
