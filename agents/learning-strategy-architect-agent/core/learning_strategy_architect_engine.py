"""
Learning Strategy Architect Engine — Learning Strategy Architect.
L&D strategy & operating model: build/buy, governance, and portfolio balance
...
class LearningStrategyArchitectEngine:
    """Deterministic engine for Learning Strategy Architect — zero network, HR-grade logic."""
    ...
    def analyze(cls, text: str, **kwargs) -> Analysis:
        ...
        # Domain heuristics for Learning Strategy Architect

        # H1: keyword coverage for learning strategy architect
        domain_keywords=['learning', 'strategy', 'architect']
        hits=sum(1 for kw in domain_keywords if kw in low)
        if hits==0 and len(low)>20:
            findings.append(Finding("learning-strategy-architect-agent","medium","Low Learning Strategy Architect signal","No core keywords (" + ", ".join(domain_keywords) + ") detected","Add explicit coverage of learning strategy architect or confirm scope is out-of-domain"))
        metrics["keyword_hits"]=hits
        # H2: thin/structured
        if metrics["words"]<12:
            findings.append(Finding("learning-strategy-architect-agent","low","Thin content","Very short input — shallow analysis","Provide 2-3 representative paragraphs or a file"))
        elif metrics["words"]>1500:
            metrics["depth"]="deep"
        else:
            metrics["depth"]="standard"
        if ":" not in t and "\n" not in t.strip():
            findings.append(Finding("learning-strategy-architect-agent","low","Unstructured input","Single-line / no key:value structure","Provide bullets, JSON, or sections for richer output"))
        # H3: PII/bias guardrails (HR-grade)
        if pii_hits>0:
            findings.append(Finding("learning-strategy-architect-agent","high","PII detected (emails)","Found " + str(pii_hits) + " email(s) in input","Scrub/anonymize PII before sharing; use placeholder IDs"))
        if found_bias:
            findings.append(Finding("learning-strategy-architect-agent","medium","Bias language flagged","Terms: " + ", ".join(found_bias),"Rewrite with inclusive, skills-based language; add structured rubric"))
        # H4: HR compliance framing
        if any(w in low for w in ["critical","urgent","blocker","grievance","termination","legal"]):
            findings.append(Finding("learning-strategy-architect-agent","high","High-stakes HR language","Input declares grievance/termination/legal scope","Loop in ER/legal, document rationale, and keep audit trail"))
        ...
        if "metric" not in low and "measure" not in low and "kpi" not in low and metrics["words"]>30:
            findings.append(Finding("learning-strategy-architect-agent","low","Measurement language absent","No metric/measure/KPI framing for an HR/L&D task","Add leading + lagging measures and how you will know it worked"))
        # H6: action vs insight balance
        if "action" not in low and "next step" not in low and metrics["words"]>40:
            findings.append(Finding("learning-strategy-architect-agent","low","Action orientation light","Few action/next-step cues in input","Close with 3 owner-dated actions"))
