"""CLI for Launch Telemetry Tracker — Pre/post metric deltas, sentiment shift, source attribution, repeat-launch playbook"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Launch Telemetry Tracker — Pre/post metric deltas, sentiment shift, source attribution, repeat-launch playbook')
    ...
    from core.launch_telemetry_engine import LaunchTelemetryEngine
    analysis = LaunchTelemetryEngine.analyze(text)
    if args.json:
        ...
        print(LaunchTelemetryEngine.format_report(analysis))
