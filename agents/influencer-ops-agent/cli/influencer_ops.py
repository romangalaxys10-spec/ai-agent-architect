"""CLI for Influencer Ops Commander — Brief generation, deliverable tracking, usage-rights ledger, payout reconciliation"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Influencer Ops Commander — Brief generation, deliverable tracking, usage-rights ledger, payout reconciliation')
    ...
    from core.influencer_ops_engine import InfluencerOpsEngine
    analysis = InfluencerOpsEngine.analyze(text)
    if args.json:
        ...
        print(InfluencerOpsEngine.format_report(analysis))
