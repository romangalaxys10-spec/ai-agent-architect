"""CLI for Linux Admin — User/group/sudo, service (systemd), cron/at, and filesystem (ext4/xfs/btrfs) operations"""
import os, sys
...
def main():
    parser=argparse.ArgumentParser(description="Linux Admin — User/group/sudo, service (systemd), cron/at, and filesystem (ext4/xfs/btrfs) operations")
    ...
    from core.linux_admin_engine import LinuxAdminEngine
    analysis=LinuxAdminEngine.analyze(text, os_hint=args.os)
    if args.json:
        ...
        print(LinuxAdminEngine.format_report(analysis))
