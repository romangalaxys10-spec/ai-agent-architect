"""CLI for Secrets & Config Auditor — Hardcoded secret scan beyond SAST, rotation schedules, least-privilege env config mapping"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Secrets & Config Auditor — Hardcoded secret scan beyond SAST, rotation schedules, least-privilege env config mapping')
    ...
    from core.secrets_vault_engine import SecretsVaultEngine
    analysis = SecretsVaultEngine.analyze(text)
    if args.json:
        ...
        print(SecretsVaultEngine.format_report(analysis))
