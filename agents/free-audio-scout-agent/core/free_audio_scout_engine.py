"""
Free Audio Scout Engine — Free Audio Scout.
Free music/SFX (Pixabay Music, Mixkit, YouTube Audio Library) with license hygiene
...
class FreeAudioScoutEngine:
    """Deterministic engine for Free Audio Scout — zero network, platform-aware logic."""
    ...
    def analyze(cls, text: str, **kwargs) -> Analysis:
        ...
        # H1: keyword coverage for free audio scout
        domain_keywords=['free', 'audio', 'scout']
        hits=sum(1 for kw in domain_keywords if kw in low)
        if hits==0 and len(low)>20:
            findings.append(Finding("free-audio-scout-agent","medium","Low Free Audio Scout signal","No core keywords (" + ", ".join(domain_keywords) + ") detected","Add explicit coverage of free audio scout or confirm scope is out-of-domain"))
        metrics["keyword_hits"]=hits
        # H2: thin/structured
        if metrics["words"]<12:
            findings.append(Finding("free-audio-scout-agent","low","Thin content","Very short input — shallow analysis","Provide 2-3 representative paragraphs or a file"))
        elif metrics["words"]>1500:
            metrics["depth"]="deep"
        else:
            metrics["depth"]="standard"
        if ":" not in t and "\n" not in t.strip():
            findings.append(Finding("free-audio-scout-agent","low","Unstructured input","Single-line / no key:value structure","Provide bullets, JSON, or sections for richer output"))
        # H3: social growth guardrails
        if not has_cta and metrics["words"]>25:
            findings.append(Finding("free-audio-scout-agent","low","CTA absent","No clear call-to-action detected for a social growth task","Add one CTA (comment/follow/link) per post"))
        if metrics["hook_hits"]==0 and "Video: Freebie + Gemini" in ["Social: LinkedIn","Social: X","Social: TikTok","Social: Instagram","Social: YouTube"]:
            findings.append(Finding("free-audio-scout-agent","low","Hook language light","No hook/CTA/pov/loop cues for a virality task","Lead with a hook in first 2 seconds / first line"))
        ...
        if "Video: Freebie + Gemini"=="Video: Freebie + Gemini":
            if "attribution" not in low and "license" not in low and metrics["words"]>20:
                findings.append(Finding("free-audio-scout-agent","medium","License hygiene absent","No attribution/license framing for freebie-sourced video","State source + license (Pexels/Pixabay/Mixkit) and keep attribution file"))
            if "gemini" not in low and "veo" not in low and "free" not in low and detected_platform=="agnostic":
                findings.append(Finding("free-audio-scout-agent","low","Freebie/Gemini signal absent","No free-stock or Gemini cue for a video-gen task","Name source (Pexels/Pixabay/Mixkit) or Gemini/VeO prompt"))
        ...
        if "Video: Freebie + Gemini".startswith("Social:") and detected_platform!="agnostic" and "Video: Freebie + Gemini".split(":")[1].strip().lower() not in detected_platform and detected_platform not in ["agnostic"]:
            ...
            if platform_signals.get(detected_platform,0)>=2:
                findings.append(Finding("free-audio-scout-agent","low","Platform mismatch hint","Input strongly signals " + detected_platform + " but agent is Video: Freebie + Gemini","Confirm target platform or route to the right agent"))
        # H6: spam/rate-limit safety
        if any(k in low for k in ["spam","scrape","bot","auto follow","auto like","bulk dm"]):
            findings.append(Finding("free-audio-scout-agent","high","Automation risk language","Input mentions bulk/scrape/bot — platform rate-limit risk","Throttle, add human review, keep ToS-compliant allowlists"))
