"""CLI for Feature Engineering Sentinel — Feature drift detection, leakage checks, importance ranking, store backfill plans"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Feature Engineering Sentinel — Feature drift detection, leakage checks, importance ranking, store backfill plans')
    ...
    from core.feature_store_engine import FeatureStoreEngine
    analysis = FeatureStoreEngine.analyze(text)
    if args.json:
        ...
        print(FeatureStoreEngine.format_report(analysis))
