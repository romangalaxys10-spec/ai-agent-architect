"""CLI for Agent Evaluation Harness — Trajectory scoring, tool-use accuracy, golden-set curation, pass@k gating"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Agent Evaluation Harness — Trajectory scoring, tool-use accuracy, golden-set curation, pass@k gating')
    ...
    from core.agent_eval_harness_engine import AgentEvalHarnessEngine
    analysis = AgentEvalHarnessEngine.analyze(text)
    if args.json:
        ...
        print(AgentEvalHarnessEngine.format_report(analysis))
