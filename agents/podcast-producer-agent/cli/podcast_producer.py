"""CLI for Podcast Producer — Episode arcs, show-note generation, guest research packs, distribution checklist"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Podcast Producer — Episode arcs, show-note generation, guest research packs, distribution checklist')
    ...
    from core.podcast_producer_engine import PodcastProducerEngine
    analysis = PodcastProducerEngine.analyze(text)
    if args.json:
        ...
        print(PodcastProducerEngine.format_report(analysis))
