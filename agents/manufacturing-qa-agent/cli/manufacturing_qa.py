"""CLI for Manufacturing QA Sentinel — SPC chart interpretation, defect Pareto, CAPA tracking, traceability mapping"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Manufacturing QA Sentinel — SPC chart interpretation, defect Pareto, CAPA tracking, traceability mapping')
    ...
    from core.manufacturing_qa_engine import ManufacturingQaEngine
    analysis = ManufacturingQaEngine.analyze(text)
    if args.json:
        ...
        print(ManufacturingQaEngine.format_report(analysis))
