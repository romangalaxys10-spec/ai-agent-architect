"""CLI for Shell Benchmark — Startup time (zsh --startup), plugin cost, and prompt latency profiling"""
import os, sys
...
def main():
    parser=argparse.ArgumentParser(description="Shell Benchmark — Startup time (zsh --startup), plugin cost, and prompt latency profiling")
    ...
    from core.shell_benchmark_engine import ShellBenchmarkEngine
    analysis=ShellBenchmarkEngine.analyze(text, os_hint=args.os)
    if args.json:
        ...
        print(ShellBenchmarkEngine.format_report(analysis))
