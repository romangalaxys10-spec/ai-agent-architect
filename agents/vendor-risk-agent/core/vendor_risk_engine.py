"""
Vendor Risk Engine — Vendor Risk Assessor.
Risk tiering, questionnaire gap analysis, mitigation plan generation
...
class VendorRiskEngine:
    """Deterministic engine for Vendor Risk Assessor — zero network, pure logic."""
    ...
    def analyze(cls, text: str, **kwargs) -> Analysis:
        ...
        # --- Domain-specific heuristics for Vendor Risk Assessor ---

        # Heuristic 1: keyword coverage for vendor risk
        domain_keywords = ['vendor', 'risk']
        hits = sum(1 for kw in domain_keywords if kw in low)
        if hits == 0 and len(low) > 20:
            findings.append(Finding("vendor-risk-agent","medium","Low Vendor Risk Assessor signal","No core keywords (" + ", ".join(domain_keywords) + ") detected in content","Add explicit coverage of vendor or confirm scope is out-of-domain"))
        metrics["keyword_hits"] = hits

        # Heuristic 2: length / depth check
        if metrics["words"] < 15:
            findings.append(Finding("vendor-risk-agent","low","Thin content","Very short input — analysis will be shallow","Provide 3-5 representative paragraphs or a file for deeper analysis"))
        ...
        if found_risks:
            findings.append(Finding("vendor-risk-agent","medium","Unresolved markers","Found placeholders: "+ ", ".join(found_risks),"Resolve or ticket them before ship"))
        ...
        if any(w in low for w in ["critical","urgent","blocker","p0","sev1"]):
            findings.append(Finding("vendor-risk-agent","high","Urgency signal present","Input declares critical/urgent scope","Confirm severity, add mitigation owner and deadline"))
            metrics["urgency"] = True

        # Heuristic 5: structure check — does input look structured?
        if ":" not in t and "\n" not in t.strip():
            findings.append(Finding("vendor-risk-agent","low","Unstructured input","Single-line / no key:value structure detected","Provide structured input (bullets, JSON, or sections) for richer output"))
        ...
        if "Ops, Finance & Legal" in ["Security & Compliance","Ops, Finance & Legal"] and "compliance" not in low and "risk" not in low:
            findings.append(Finding("vendor-risk-agent","low","Compliance language absent","No compliance/risk framing found for a Ops, Finance & Legal task","State assumptions, scope, and non-advice disclaimer where needed"))
