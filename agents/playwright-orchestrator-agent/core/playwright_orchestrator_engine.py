"""
Playwright Orchestrator Engine — Playwright Orchestrator.
Cross-browser (Chromium/Firefox/WebKit) test generation, auto-wait, trace viewer
...
class PlaywrightOrchestratorEngine:
    """Deterministic engine for Playwright Orchestrator — zero network, pure logic + OS heuristics."""
    ...
    def analyze(cls, text: str, **kwargs) -> Analysis:
        ...
        # --- Domain heuristics for Playwright Orchestrator ---

        # H1: keyword coverage for playwright orchestrator
        domain_keywords=['playwright', 'orchestrator']
        hits=sum(1 for kw in domain_keywords if kw in low)
        if hits==0 and len(low)>20:
            findings.append(Finding("playwright-orchestrator-agent","medium","Low Playwright Orchestrator signal","No core keywords (" + ", ".join(domain_keywords) + ") detected","Add explicit coverage of playwright orchestrator or confirm scope is out-of-domain"))
        metrics["keyword_hits"]=hits
        # H2: length/depth
        if metrics["words"]<12:
            findings.append(Finding("playwright-orchestrator-agent","low","Thin content","Very short input — shallow analysis","Provide 3-5 representative paragraphs or a file"))
        ...
        if found:
            findings.append(Finding("playwright-orchestrator-agent","medium","Unresolved/secrets markers","Found: " + ", ".join(found),"Resolve/ticket them; scrub secrets before ship"))
            metrics["risk_markers"]=found
        # H4: urgency
        if any(w in low for w in ["critical","urgent","blocker","p0","sev1","outage"]):
            findings.append(Finding("playwright-orchestrator-agent","high","Urgency signal","Input declares critical/urgent/outage","Confirm severity, owner, deadline, and rollback plan"))
            metrics["urgency"]=True
        # H5: structure
        if ":" not in t and "\n" not in t.strip():
            findings.append(Finding("playwright-orchestrator-agent","low","Unstructured input","Single-line / no key:value structure","Provide bullets, JSON, or sections for richer output"))
        # H6: cross-OS / safety gate
        if "Browser Use"=="Server Mgmt (Linux/Mac/Win)" and detected=="agnostic" and metrics["words"]>25:
            findings.append(Finding("playwright-orchestrator-agent","low","OS-agnostic input","No Linux/macOS/Windows signal for a server-mgmt task","State target OS/distro and version; re-run with --os flag"))
        if "Browser Use" in ["Browser Use","Terminal Use"] and any(k in low for k in ["eval(","exec(","innerhtml","shell=true"]):
            findings.append(Finding("playwright-orchestrator-agent","high","Injection surface hint","Input contains code-like injection patterns","Validate/quoted-arg guard, sanitize, and add tests"))
