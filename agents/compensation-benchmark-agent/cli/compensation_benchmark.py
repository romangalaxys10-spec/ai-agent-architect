"""CLI for Compensation Benchmarker — Band mapping, market percentile analysis, pay-equity flags, offer construction (not advice)"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Compensation Benchmarker — Band mapping, market percentile analysis, pay-equity flags, offer construction (not advice)')
    ...
    from core.compensation_benchmark_engine import CompensationBenchmarkEngine
    analysis = CompensationBenchmarkEngine.analyze(text)
    if args.json:
        ...
        print(CompensationBenchmarkEngine.format_report(analysis))
