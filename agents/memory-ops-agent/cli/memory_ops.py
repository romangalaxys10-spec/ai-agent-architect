"""CLI for Memory Ops Auditor — Memory tier health, compaction trigger tuning, recall precision measurement, curation backlog"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Memory Ops Auditor — Memory tier health, compaction trigger tuning, recall precision measurement, curation backlog')
    ...
    from core.memory_ops_engine import MemoryOpsEngine
    analysis = MemoryOpsEngine.analyze(text)
    if args.json:
        ...
        print(MemoryOpsEngine.format_report(analysis))
