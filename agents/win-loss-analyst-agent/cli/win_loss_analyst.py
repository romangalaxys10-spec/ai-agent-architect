"""CLI for Win/Loss Analyst — Interview synthesis, loss-reason taxonomy, competitive loss attribution, fix-backlog routing"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Win/Loss Analyst — Interview synthesis, loss-reason taxonomy, competitive loss attribution, fix-backlog routing')
    ...
    from core.win_loss_analyst_engine import WinLossAnalystEngine
    analysis = WinLossAnalystEngine.analyze(text)
    if args.json:
        ...
        print(WinLossAnalystEngine.format_report(analysis))
