"""CLI for Anomaly Hunter — Time-series outlier detection, seasonal decomposition, root-cause ranking, alert suppression tuning"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Anomaly Hunter — Time-series outlier detection, seasonal decomposition, root-cause ranking, alert suppression tuning')
    ...
    from core.anomaly_hunter_engine import AnomalyHunterEngine
    analysis = AnomalyHunterEngine.analyze(text)
    if args.json:
        ...
        print(AnomalyHunterEngine.format_report(analysis))
