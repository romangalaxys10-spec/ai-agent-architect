"""CLI for Chaos Engineering Designer — Fault injection plans, blast-radius scoping, steady-state hypotheses, game-day runbooks"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Chaos Engineering Designer — Fault injection plans, blast-radius scoping, steady-state hypotheses, game-day runbooks')
    ...
    from core.chaos_lab_engine import ChaosLabEngine
    analysis = ChaosLabEngine.analyze(text)
    if args.json:
        ...
        print(ChaosLabEngine.format_report(analysis))
