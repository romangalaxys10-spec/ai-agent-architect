"""CLI for Climate Risk Analyst — Physical/transition risk scoring, disclosure mapping (TCFD/ISSB), mitigation ranking"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Climate Risk Analyst — Physical/transition risk scoring, disclosure mapping (TCFD/ISSB), mitigation ranking')
    ...
    from core.climate_risk_engine import ClimateRiskEngine
    analysis = ClimateRiskEngine.analyze(text)
    if args.json:
        ...
        print(ClimateRiskEngine.format_report(analysis))
