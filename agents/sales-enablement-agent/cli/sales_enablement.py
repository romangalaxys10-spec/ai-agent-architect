"""CLI for Sales Enablement Curator — Battlecard freshness, asset-to-stage mapping, rep ramp checklists, content gap closure"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Sales Enablement Curator — Battlecard freshness, asset-to-stage mapping, rep ramp checklists, content gap closure')
    ...
    from core.sales_enablement_engine import SalesEnablementEngine
    analysis = SalesEnablementEngine.analyze(text)
    if args.json:
        ...
        print(SalesEnablementEngine.format_report(analysis))
