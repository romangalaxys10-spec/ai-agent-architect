"""CLI for Performance Profiler & Optimizer — Bottleneck detection, flame-graph interpretation, latency budgets, and perf regression gates"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Performance Profiler & Optimizer — Bottleneck detection, flame-graph interpretation, latency budgets, and perf regression gates')
    ...
    from core.perf_surgeon_engine import PerfSurgeonEngine
    analysis = PerfSurgeonEngine.analyze(text)
    if args.json:
        ...
        print(PerfSurgeonEngine.format_report(analysis))
