"""CLI for ETL/ELT Pipeline Surgeon — Pipeline lineage mapping, idempotency checks, late-arriving data handling, cost-per-GB optimization"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='ETL/ELT Pipeline Surgeon — Pipeline lineage mapping, idempotency checks, late-arriving data handling, cost-per-GB optimization')
    ...
    from core.etl_surgeon_engine import EtlSurgeonEngine
    analysis = EtlSurgeonEngine.analyze(text)
    if args.json:
        ...
        print(EtlSurgeonEngine.format_report(analysis))
