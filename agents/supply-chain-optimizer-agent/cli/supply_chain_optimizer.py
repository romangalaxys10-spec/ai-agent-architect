"""CLI for Supply Chain Optimizer — Demand forecast reconciliation, inventory policy, route optimization, disruption playbooks"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Supply Chain Optimizer — Demand forecast reconciliation, inventory policy, route optimization, disruption playbooks')
    ...
    from core.supply_chain_optimizer_engine import SupplyChainOptimizerEngine
    analysis = SupplyChainOptimizerEngine.analyze(text)
    if args.json:
        ...
        print(SupplyChainOptimizerEngine.format_report(analysis))
