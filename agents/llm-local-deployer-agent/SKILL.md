---
name: llm-local-deployer-agent
description: 'Ollama/vLLM/llama.cpp local deploy with model registry'
version: 1.0.0
...
2. Run deterministic heuristics for **LLM Local Deployer** — severity-ranked findings with evidence + fix (secret exposure, urgency, domain-specific hygiene).
...
## 🔌 I/O Contract
- **CLI:** `python agents/llm-local-deployer-agent/cli/llm_local_deployer.py --help`
