"""CLI for Feature Flag Governor — Flag lifecycle, targeting rules, kill-switch runbooks, experiment exposure auditing"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Feature Flag Governor — Flag lifecycle, targeting rules, kill-switch runbooks, experiment exposure auditing')
    ...
    from core.feature_flag_engine import FeatureFlagEngine
    analysis = FeatureFlagEngine.analyze(text)
    if args.json:
        ...
        print(FeatureFlagEngine.format_report(analysis))
