"""CLI for Package Manager Pilot — apt/dnf/pacman/apk/zypper — repo pinning, hold/unhold, and atomic rollback"""
import os, sys
...
def main():
    parser=argparse.ArgumentParser(description="Package Manager Pilot — apt/dnf/pacman/apk/zypper — repo pinning, hold/unhold, and atomic rollback")
    ...
    from core.package_manager_engine import PackageManagerEngine
    analysis=PackageManagerEngine.analyze(text, os_hint=args.os)
    if args.json:
        ...
        print(PackageManagerEngine.format_report(analysis))
