"""CLI for Windows Security Sentinel — Defender, AppLocker/WDAC, Credential Guard, and event log forensics"""
import os, sys
...
def main():
    parser=argparse.ArgumentParser(description="Windows Security Sentinel — Defender, AppLocker/WDAC, Credential Guard, and event log forensics")
    ...
    from core.windows_security_engine import WindowsSecurityEngine
    analysis=WindowsSecurityEngine.analyze(text, os_hint=args.os)
    if args.json:
        ...
        print(WindowsSecurityEngine.format_report(analysis))
