"""CLI for Linux Net Debug — tcpdump/Wireshark, conntrack, and `ss`/`ip` forensics"""
import os, sys
...
def main():
    parser=argparse.ArgumentParser(description="Linux Net Debug — tcpdump/Wireshark, conntrack, and `ss`/`ip` forensics")
    ...
    from core.linux_net_debug_engine import LinuxNetDebugEngine
    analysis=LinuxNetDebugEngine.analyze(text)
    if args.json:
        ...
        print(LinuxNetDebugEngine.format_report(analysis))
