"""CLI for Gov Procurement Navigator — RFP eligibility checks, compliance matrix, proposal choreography for public sector"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Gov Procurement Navigator — RFP eligibility checks, compliance matrix, proposal choreography for public sector')
    ...
    from core.gov_procurement_engine import GovProcurementEngine
    analysis = GovProcurementEngine.analyze(text)
    if args.json:
        ...
        print(GovProcurementEngine.format_report(analysis))
