"""CLI for Insurance Coverage Auditor — Gap analysis, limit adequacy, exclusion flagging, renewal negotiation prep (not advice)"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Insurance Coverage Auditor — Gap analysis, limit adequacy, exclusion flagging, renewal negotiation prep (not advice)')
    ...
    from core.insurance_advisor_engine import InsuranceAdvisorEngine
    analysis = InsuranceAdvisorEngine.analyze(text)
    if args.json:
        ...
        print(InsuranceAdvisorEngine.format_report(analysis))
