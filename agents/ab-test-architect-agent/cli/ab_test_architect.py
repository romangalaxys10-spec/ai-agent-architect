"""CLI for A/B Experiment Architect — Hypothesis framing, sample-size math, guardrail metrics, ship/hold/iterate verdicts"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='A/B Experiment Architect — Hypothesis framing, sample-size math, guardrail metrics, ship/hold/iterate verdicts')
    ...
    from core.ab_test_architect_engine import AbTestArchitectEngine
    analysis = AbTestArchitectEngine.analyze(text)
    if args.json:
        ...
        print(AbTestArchitectEngine.format_report(analysis))
