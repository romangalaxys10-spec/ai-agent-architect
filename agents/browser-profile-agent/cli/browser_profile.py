"""CLI for Browser Profile Steward — Profile cloning, fingerprint hygiene, proxy rotation, and container isolation"""
import os, sys
...
def main():
    parser=argparse.ArgumentParser(description="Browser Profile Steward — Profile cloning, fingerprint hygiene, proxy rotation, and container isolation")
    ...
    from core.browser_profile_engine import BrowserProfileEngine
    analysis=BrowserProfileEngine.analyze(text, os_hint=args.os)
    if args.json:
        ...
        print(BrowserProfileEngine.format_report(analysis))
