---
name: x-thread-architect-agent
description: 'Hook + pacing + cliffhangers for long-form X threads'
version: 1.0.0
...
2. Run deterministic heuristics for **X Thread Architect** — severity-ranked findings with evidence + fix (platform mismatch, CTA/hook, license hygiene for freebie, Gemini prompt hygiene).
...
## 🔌 I/O Contract
- **CLI:** `python agents/x-thread-architect-agent/cli/x_thread_architect.py --help`
