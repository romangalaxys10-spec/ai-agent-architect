"""
Privacy Shield Engine — Privacy & DPA Steward.
GDPR/CCPA gap analysis, DPA clause mapping, consent-flow audits, retention enforcement
...
class PrivacyShieldEngine:
    """Deterministic engine for Privacy & DPA Steward — zero network, pure logic."""
    ...
    def analyze(cls, text: str, **kwargs) -> Analysis:
        ...
        # --- Domain-specific heuristics for Privacy & DPA Steward ---

        # Heuristic 1: keyword coverage for privacy shield
        domain_keywords = ['privacy', 'shield']
        hits = sum(1 for kw in domain_keywords if kw in low)
        if hits == 0 and len(low) > 20:
            findings.append(Finding("privacy-shield-agent","medium","Low Privacy & DPA Steward signal","No core keywords (" + ", ".join(domain_keywords) + ") detected in content","Add explicit coverage of privacy or confirm scope is out-of-domain"))
        metrics["keyword_hits"] = hits

        # Heuristic 2: length / depth check
        if metrics["words"] < 15:
            findings.append(Finding("privacy-shield-agent","low","Thin content","Very short input — analysis will be shallow","Provide 3-5 representative paragraphs or a file for deeper analysis"))
        ...
        if found_risks:
            findings.append(Finding("privacy-shield-agent","medium","Unresolved markers","Found placeholders: "+ ", ".join(found_risks),"Resolve or ticket them before ship"))
        ...
        if any(w in low for w in ["critical","urgent","blocker","p0","sev1"]):
            findings.append(Finding("privacy-shield-agent","high","Urgency signal present","Input declares critical/urgent scope","Confirm severity, add mitigation owner and deadline"))
            metrics["urgency"] = True

        # Heuristic 5: structure check — does input look structured?
        if ":" not in t and "\n" not in t.strip():
            findings.append(Finding("privacy-shield-agent","low","Unstructured input","Single-line / no key:value structure detected","Provide structured input (bullets, JSON, or sections) for richer output"))
        ...
        if "Security & Compliance" in ["Security & Compliance","Ops, Finance & Legal"] and "compliance" not in low and "risk" not in low:
            findings.append(Finding("privacy-shield-agent","low","Compliance language absent","No compliance/risk framing found for a Security & Compliance task","State assumptions, scope, and non-advice disclaimer where needed"))
