"""CLI for Newsletter Architect — Section planning, curation scoring, subject-A/B generation, send-time optimization"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Newsletter Architect — Section planning, curation scoring, subject-A/B generation, send-time optimization')
    ...
    from core.newsletter_architect_engine import NewsletterArchitectEngine
    analysis = NewsletterArchitectEngine.analyze(text)
    if args.json:
        ...
        print(NewsletterArchitectEngine.format_report(analysis))
