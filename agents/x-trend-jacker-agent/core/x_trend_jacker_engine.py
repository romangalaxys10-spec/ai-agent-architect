"""
X Trend Jacker Engine — X Trend Jacker.
...
class XTrendJackerEngine:
    """Deterministic engine for X Trend Jacker — zero network, platform-aware logic."""
    ...
    def analyze(cls, text: str, **kwargs) -> Analysis:
        ...
        # H1: keyword coverage for x trend jacker
        domain_keywords=['x', 'trend', 'jacker']
        hits=sum(1 for kw in domain_keywords if kw in low)
        if hits==0 and len(low)>20:
            findings.append(Finding("x-trend-jacker-agent","medium","Low X Trend Jacker signal","No core keywords (" + ", ".join(domain_keywords) + ") detected","Add explicit coverage of x trend jacker or confirm scope is out-of-domain"))
        metrics["keyword_hits"]=hits
        # H2: thin/structured
        if metrics["words"]<12:
            findings.append(Finding("x-trend-jacker-agent","low","Thin content","Very short input — shallow analysis","Provide 2-3 representative paragraphs or a file"))
        elif metrics["words"]>1500:
            metrics["depth"]="deep"
        else:
            metrics["depth"]="standard"
        if ":" not in t and "\n" not in t.strip():
            findings.append(Finding("x-trend-jacker-agent","low","Unstructured input","Single-line / no key:value structure","Provide bullets, JSON, or sections for richer output"))
        # H3: social growth guardrails
        if not has_cta and metrics["words"]>25:
            findings.append(Finding("x-trend-jacker-agent","low","CTA absent","No clear call-to-action detected for a social growth task","Add one CTA (comment/follow/link) per post"))
        if metrics["hook_hits"]==0 and "Social: X" in ["Social: LinkedIn","Social: X","Social: TikTok","Social: Instagram","Social: YouTube"]:
            findings.append(Finding("x-trend-jacker-agent","low","Hook language light","No hook/CTA/pov/loop cues for a virality task","Lead with a hook in first 2 seconds / first line"))
        ...
        if "Social: X"=="Video: Freebie + Gemini":
            if "attribution" not in low and "license" not in low and metrics["words"]>20:
                findings.append(Finding("x-trend-jacker-agent","medium","License hygiene absent","No attribution/license framing for freebie-sourced video","State source + license (Pexels/Pixabay/Mixkit) and keep attribution file"))
            if "gemini" not in low and "veo" not in low and "free" not in low and detected_platform=="agnostic":
                findings.append(Finding("x-trend-jacker-agent","low","Freebie/Gemini signal absent","No free-stock or Gemini cue for a video-gen task","Name source (Pexels/Pixabay/Mixkit) or Gemini/VeO prompt"))
        ...
        if "Social: X".startswith("Social:") and detected_platform!="agnostic" and "Social: X".split(":")[1].strip().lower() not in detected_platform and detected_platform not in ["agnostic"]:
            ...
            if platform_signals.get(detected_platform,0)>=2:
                findings.append(Finding("x-trend-jacker-agent","low","Platform mismatch hint","Input strongly signals " + detected_platform + " but agent is Social: X","Confirm target platform or route to the right agent"))
        # H6: spam/rate-limit safety
        if any(k in low for k in ["spam","scrape","bot","auto follow","auto like","bulk dm"]):
            findings.append(Finding("x-trend-jacker-agent","high","Automation risk language","Input mentions bulk/scrape/bot — platform rate-limit risk","Throttle, add human review, keep ToS-compliant allowlists"))
