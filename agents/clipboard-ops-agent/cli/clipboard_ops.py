"""CLI for Clipboard Ops — Multi-format clipboard (text/image/files), history vault, and cross-app paste validation"""
import os, sys
...
def main():
    parser=argparse.ArgumentParser(description="Clipboard Ops — Multi-format clipboard (text/image/files), history vault, and cross-app paste validation")
    ...
    from core.clipboard_ops_engine import ClipboardOpsEngine
    analysis=ClipboardOpsEngine.analyze(text, os_hint=args.os)
    if args.json:
        ...
        print(ClipboardOpsEngine.format_report(analysis))
