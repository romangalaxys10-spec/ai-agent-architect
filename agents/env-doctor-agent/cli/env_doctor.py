"""CLI for Environment Doctor — Repro pass/fail for .env, Docker, Node/Python version mismatches, setup script generation"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Environment Doctor — Repro pass/fail for .env, Docker, Node/Python version mismatches, setup script generation')
    ...
    from core.env_doctor_engine import EnvDoctorEngine
    analysis = EnvDoctorEngine.analyze(text)
    if args.json:
        ...
        print(EnvDoctorEngine.format_report(analysis))
