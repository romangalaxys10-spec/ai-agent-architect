"""CLI for Adversarial Tester — Red-team prompt banks, bypass attempt logging, safety regression tracking"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Adversarial Tester — Red-team prompt banks, bypass attempt logging, safety regression tracking')
    ...
    from core.adversarial_tester_engine import AdversarialTesterEngine
    analysis = AdversarialTesterEngine.analyze(text)
    if args.json:
        ...
        print(AdversarialTesterEngine.format_report(analysis))
