"""CLI for Log Forensics & Observability — Structured log aggregation, anomaly detection, distributed trace stitching, alert synthesis"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Log Forensics & Observability — Structured log aggregation, anomaly detection, distributed trace stitching, alert synthesis')
    ...
    from core.log_detective_engine import LogDetectiveEngine
    analysis = LogDetectiveEngine.analyze(text)
    if args.json:
        ...
        print(LogDetectiveEngine.format_report(analysis))
