"""
SSR Optimizer Engine — SSR Optimizer.
SSR/SSG/CSR tradeoffs, hydration, and streaming
...
class SsrOptimizerEngine:
    """Deterministic engine for SSR Optimizer — zero network, sysadmin logic."""
    ...
    def analyze(cls, text: str, **kwargs) -> Analysis:
        ...
        # H1: keyword coverage for ssr optimizer
        domain_keywords=['ssr', 'optimizer']
        hits=sum(1 for kw in domain_keywords if kw in low)
        if hits==0 and len(low)>20:
            findings.append(Finding("ssr-optimizer-agent","medium","Low SSR Optimizer signal","No core keywords (" + ", ".join(domain_keywords) + ") detected","Add explicit coverage of ssr optimizer or confirm scope is out-of-domain"))
        metrics["keyword_hits"]=hits
        # H2: thin/structured
        if metrics["words"]<12:
            findings.append(Finding("ssr-optimizer-agent","low","Thin content","Very short input — shallow analysis","Provide 2-3 representative paragraphs or a file"))
        elif metrics["words"]>1500:
            metrics["depth"]="deep"
        else:
            metrics["depth"]="standard"
        if ":" not in t and "\n" not in t.strip():
            findings.append(Finding("ssr-optimizer-agent","low","Unstructured input","Single-line / no key:value structure","Provide bullets, JSON, or sections for richer output"))
        ...
        if any(k in low for k in ["private key","mnemonic","seed phrase","secret key"]):
            findings.append(Finding("ssr-optimizer-agent","critical","Secret exposure","Private key/mnemonic in input","Redact immediately, rotate, and use env/secret manager"))
        if "password" in low and "password" in "ssr-optimizer-agent":
            pass
        if "SysAdmin: Web Dev" in ["SysAdmin: Security/Optimization"] and "cve" not in low and "patch" not in low and metrics["words"]>25:
            findings.append(Finding("ssr-optimizer-agent","low","Patch hygiene light","No CVE/patch framing for a security task","Add CVE, EPSS, and patch SLA"))
        if "SysAdmin: Web Dev"=="SysAdmin: Local LLM" and not any(k in low for k in ["vram","quant","latency","throughput","ttft"]):
            findings.append(Finding("ssr-optimizer-agent","low","Perf framing absent","No VRAM/quant/latency framing for a local LLM perf task","Add TTFT/TPS, VRAM, and quant tradeoff"))
        # H4: urgency
        if any(w in low for w in ["critical","urgent","blocker","p0","sev1","outage","exploit"]):
            findings.append(Finding("ssr-optimizer-agent","high","Urgency signal","Input declares critical/urgent/outage/exploit","Confirm severity, owner, deadline, and rollback"))
        ...
        if "SysAdmin: Web Dev"=="SysAdmin: Solana" and sys_signals.get("solana",0)==0 and metrics["words"]>25:
            findings.append(Finding("ssr-optimizer-agent","low","Solana signal absent","No solana/anchor/spl cue for a Solana task","Confirm chain target or route to EVM/general agent"))
