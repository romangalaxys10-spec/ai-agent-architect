"""CLI for Influencer & Creator Scout — Audience overlap scoring, authenticity checks, brief generation, performance attribution"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Influencer & Creator Scout — Audience overlap scoring, authenticity checks, brief generation, performance attribution')
    ...
    from core.influencer_scout_engine import InfluencerScoutEngine
    analysis = InfluencerScoutEngine.analyze(text)
    if args.json:
        ...
        print(InfluencerScoutEngine.format_report(analysis))
