"""CLI for Design System Curator — Token hygiene, component API consistency, Figma drift detection, adoption metrics"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Design System Curator — Token hygiene, component API consistency, Figma drift detection, adoption metrics')
    ...
    from core.design_system_engine import DesignSystemEngine
    analysis = DesignSystemEngine.analyze(text)
    if args.json:
        ...
        print(DesignSystemEngine.format_report(analysis))
