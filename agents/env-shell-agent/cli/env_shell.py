"""CLI for Env Shell Steward — Dotfile management, profile layering (.zshrc/.bashrc), env diffing, and idempotent bootstrap"""
import os, sys
...
def main():
    parser=argparse.ArgumentParser(description="Env Shell Steward — Dotfile management, profile layering (.zshrc/.bashrc), env diffing, and idempotent bootstrap")
    ...
    from core.env_shell_engine import EnvShellEngine
    analysis=EnvShellEngine.analyze(text, os_hint=args.os)
    if args.json:
        ...
        print(EnvShellEngine.format_report(analysis))
