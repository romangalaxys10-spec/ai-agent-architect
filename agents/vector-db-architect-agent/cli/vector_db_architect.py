"""CLI for Vector DB Architect — Index selection (HNSW/IVF), chunking strategy, ANN recall benchmarking, hybrid search tuning"""
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description='Vector DB Architect — Index selection (HNSW/IVF), chunking strategy, ANN recall benchmarking, hybrid search tuning')
    ...
    from core.vector_db_architect_engine import VectorDbArchitectEngine
    analysis = VectorDbArchitectEngine.analyze(text)
    if args.json:
        ...
        print(VectorDbArchitectEngine.format_report(analysis))
