"""
Cinema 4D Motion Lab Engine — Cinema 4D Motion Lab.
Cinema 4D motion graphics with MoGraph, Redshift, and fields
...
class Cinema4dMotionEngine:
    """Deterministic engine for Cinema 4D Motion Lab — zero network, studio logic."""
    ...
    def analyze(cls, text: str, **kwargs) -> Analysis:
        ...
        # H1: keyword coverage for cinema4d motion
        domain_keywords=['cinema4d', 'motion']
        hits=sum(1 for kw in domain_keywords if kw in low)
        if hits==0 and len(low)>20:
            findings.append(Finding("cinema4d-motion-agent","medium","Low Cinema 4D Motion Lab signal","No core keywords (" + ", ".join(domain_keywords) + ") detected","Add explicit coverage of cinema4d motion or confirm scope is out-of-domain"))
        metrics["keyword_hits"]=hits
        # H2: thin/structured
        if metrics["words"]<12:
            findings.append(Finding("cinema4d-motion-agent","low","Thin content","Very short input — shallow analysis","Provide 2-3 representative paragraphs or a file"))
        elif metrics["words"]>1500:
            metrics["depth"]="deep"
        else:
            metrics["depth"]="standard"
        if ":" not in t and "\n" not in t.strip():
            findings.append(Finding("cinema4d-motion-agent","low","Unstructured input","Single-line / no key:value structure","Provide bullets, JSON, or sections for richer output"))
        # H3: render/license for studio
        if "cinema4d-motion-agent" in ["blender-studio-agent","three-studio-max-agent","cinema4d-motion-agent","houdini-fx-agent","3d-modeling-architect-agent","3d-texturing-agent","3d-render-optimizer-agent"] and "license" not in low and "render" not in low and metrics["words"]>25:
            findings.append(Finding("cinema4d-motion-agent","low","Render/license hygiene light","No render/license framing for 3D task","State renderer (Cycles/Arnold/Redshift) and asset license"))
        if "cinema4d-motion-agent" in ["video-editor-pro-agent","premiere-workflow-agent","davinci-resolve-agent","capcut-template-agent","subtitle-transcription-agent","youtube-remix-engine-agent","youtube-publisher-agent"] and not has_cta and metrics["words"]>30:
            findings.append(Finding("cinema4d-motion-agent","low","Publish CTA light","No schedule/premiere/subscribe CTA for publish task","Add CTA/end-screen/premiere plan"))
        if "copyright" in low or "piracy" in low:
            findings.append(Finding("cinema4d-motion-agent","high","Copyright risk language","Input mentions copyright/piracy","Ensure cleared sources, keep attribution"))
