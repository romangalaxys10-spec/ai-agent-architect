"""CLI for macOS Security Sentinel — Gatekeeper, XProtect, TCC db, keychain, and Endpoint Security framework"""
import os, sys
...
def main():
    parser=argparse.ArgumentParser(description="macOS Security Sentinel — Gatekeeper, XProtect, TCC db, keychain, and Endpoint Security framework")
    ...
    from core.macos_security_engine import MacosSecurityEngine
    analysis=MacosSecurityEngine.analyze(text, os_hint=args.os)
    if args.json:
        ...
        print(MacosSecurityEngine.format_report(analysis))
